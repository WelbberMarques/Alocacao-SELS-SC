...
class App(tk.Tk):
    ...
    def _sincronizar_historico(self):
        """Tenta enviar registros locais pendentes para o Supabase."""
        import threading as _th
        def _sync():
            try:
                from auth import sincronizar_historico_local
                sincronizar_historico_local()
            except Exception:
                pass
    ...
    def _build_footer(self):
        ...
        self.after(2000, self._sincronizar_historico)
        tk.Label(footer,
    ...
    def _checar_conexao(self):
        """Verifica conexao com Supabase a cada 30s."""
        def check():
            try:
                import requests as _req
                from auth import SUPABASE_URL, SUPABASE_KEY
                r = _req.get(f"{SUPABASE_URL}/rest/v1/",
                             headers={"apikey": SUPABASE_KEY}, timeout=4)
                online = r.status_code in (200, 400)
            except Exception:
                online = False
